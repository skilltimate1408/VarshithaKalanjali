...
export default function StudentKiosk() {
  ...
  const lookup = useCallback(async (ticket: string) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setResults(data.students.slice(0, 1))
  ...
  const handleKey = useCallback((key: string) => {
    if (key === '⌫') {
      ...
      const val = (inputValue + key).toUpperCase()
      setInputValue(val)
      bufferRef.current = val
    }
  }, [inputValue, lookup])
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 flex flex-col text-white">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center px-3 py-3 gap-3">
        ...
        {!loading && !error && !hasResult && !inputValue && (
          <div className="text-center text-blue-300">
            ...
            <p className="text-lg mt-1 text-blue-400">Scan barcode or tap input to type</p>
          </div>
        )}
      ...
      {/* ON-SCREEN KEYBOARD */}
      {showKeyboard && (
        <div className="shrink-0 bg-gray-900 border-t-2 border-blue-500 px-2 py-2">
          <div className="flex justify-between items-center mb-2 px-1">
            <span className="text-yellow-300 font-mono text-lg tracking-widest">
              {inputValue || <span className="text-gray-500">Type hall ticket...</span>}
            </span>
            <button
              onClick={() => setShowKeyboard(false)}
              className="text-gray-400 text-sm px-3 py-1 rounded bg-gray-700 hover:bg-gray-600"
            >
              Close ✕
            </button>
          </div>
            ...
            <div key={ri} className="flex justify-center gap-1 mb-1">
              {row.map(key => (
                <button
                  ...
                  className={`
                    h-12 rounded-lg font-bold text-lg select-none
                    ${key === '✓'
                      ? 'bg-green-500 text-white px-5 hover:bg-green-400'
                      : key === '⌫'
                      ? 'bg-red-500 text-white px-4 hover:bg-red-400'
                      : 'bg-gray-700 text-white px-3 hover:bg-gray-600 active:bg-blue-500 min-w-[42px]'
                    }
                  `}
                >
                  {key}
